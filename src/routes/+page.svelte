<script lang="ts">
		...
		if (stage === 'otp') {
			...
			if (status === 'error') {
				return {
					...
					text:
						errorMessage ??
						`Enter the ${OTP_LENGTH}-character code we sent to confirm your identity.`
				};
			}
	...
	async function handleOtpSubmit(event: SubmitEvent) {
		...
		if (!isOtpComplete) {
			status = 'error';
			errorMessage = `Enter the ${OTP_LENGTH}-character code we sent to confirm your identity.`;
			return;
		}
...
<div class="flex min-h-screen items-center justify-center bg-background px-4 py-12">
	<Card class="w-full max-w-md border border-border/60 px-6">
		<CardHeader class="space-y-2">
			...
			<CardDescription>
				Choose how you want to receive your login code. We&apos;ll send a single-use code that
				expires quickly.
			</CardDescription>
		</CardHeader>
				...
				<Tabs.Content value="email" class="mt-4">
					{#if stage === 'identifier'}
						<form class="space-y-5" onsubmit={handleIdentifierSubmit}>
							...
							>
								{identifierSubmitting ? 'Sending...' : 'Send code'}
							</Button>
						</form>
					{:else}
						...
						{#if tabIsActive}
							<form class="space-y-5" onsubmit={handleOtpSubmit}>
								...
								<Button type="submit" class="w-full" disabled={!isOtpComplete || otpSubmitting}>
		...
		<CardFooter class="justify-center">
			<p class="text-xs text-muted-foreground">
				Need help? <a class="text-primary hover:underline" href="mailto:support@example.com"
